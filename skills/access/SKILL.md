...
`~/.claude/channels/line/access.json`. You never talk to LINE — you
...
```json
{
  ...
  "allowFrom": ["Uxxxx", ...],
  "groups": {
    "Cxxxx": { "requireMention": true, "allowFrom": [] }
  },
  "pending": {
    "<6-char-code>": {
      "senderId": "...", "chatId": "...",
...
`textChunkLimit`, `chunkMode`, `mentionPatterns`, `replyFormat`. Validate types:
- `ackReaction`: string (emoji) or `""` to disable
- `replyToMode`: `off` | `first` | `all`
- `textChunkLimit`: number
- `chunkMode`: `length` | `newline`
- `mentionPatterns`: JSON array of regex strings
- `replyFormat`: `text` | `flex` (default: `text`). When set to `flex`, Claude uses Flex Messages with structured bubble layout instead of plain text.
...
- LINE user IDs start with `U` (e.g. `U1234567890abcdef...`), group IDs
  start with `C`, room IDs start with `R`. Don't confuse these.
